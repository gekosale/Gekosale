{% extends "layout.tpl" %}
{% block content %}
<h2><img src="{{ DESIGNPATH }}_images_panel/icons/modules/atributes-edit.png" alt=""/>{% trans %}TXT_PRODUCT_ATTRIBUTES{% endtrans %}: {{ currentGroup.name }}</h2>
<ul class="possibilities">
 	<li><a href="{{ URL }}{{ CURRENT_CONTROLLER }}/add" class="button" title="{% trans %}TXT_ADD_TECHNICAL_DATA_SET{% endtrans %}"><span><img src="{{ DESIGNPATH }}_images_panel/icons/buttons/add.png" alt=""/>{% trans %}TXT_ADD_TECHNICAL_DATA_SET{% endtrans %}</span></a></li>
...
GCore.OnLoad(function() {
	$('a[href="{{ URL }}{{ CURRENT_CONTROLLER }}/add"]').click(function() {
		GPrompt('{% trans %}TXT_ENTER_NEW_TECHNICAL_DATASET_NAME{% endtrans %}', function(sName) {
			GCore.StartWaiting();
			xajax_AddGroup({
				name: sName
...
<div class="block">
	<div class="scrollable-tabs">
		...
		{% for group in existingGroups %}
			<li{% if currentGroup.id == group.id %} class="active"{% endif %}><a href="{{ URL }}{{ CURRENT_CONTROLLER }}/edit/{{ group.id }}">{{ group.name }}</a></li>
		{% endfor %}
		</ul>
	</div>
	{{ form }}
