...
abstract class Availability implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Component\\Configuration\\Model\\Availability\\Map\\AvailabilityTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Component\Configuration\Model\Availability\Base\Availability object.
     * @see applyDefaults()
     */
     ...
     * @return   \Gekosale\Component\Configuration\Model\Availability\Availability The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[AvailabilityTableMap::ID] = true;
        }
    ...
    public function setAdddate($v)
    {
        ...
        if ($this->add_date !== null || $dt !== null) {
            if ($dt !== $this->add_date) {
                $this->add_date = $dt;
                $this->modifiedColumns[AvailabilityTableMap::ADD_DATE] = true;
            }
    ...
    public function setValue($v)
    {
        ...
        if ($this->value !== $v) {
            $this->value = $v;
            $this->modifiedColumns[AvailabilityTableMap::VALUE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AvailabilityTableMap::translateFieldName('Adddate', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->add_date = (null !== $col) ? PropelDateTime::newInstance($col, null, '\DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AvailabilityTableMap::translateFieldName('Value', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException("Error populating \Gekosale\Component\Configuration\Model\Availability\Availability object", 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->availabilityI18nsScheduledForDeletion !== null) {
                if (!$this->availabilityI18nsScheduledForDeletion->isEmpty()) {
                    \Gekosale\Component\Configuration\Model\Availability\AvailabilityI18nQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[AvailabilityTableMap::ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . AvailabilityTableMap::ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(AvailabilityTableMap::ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(AvailabilityTableMap::ADD_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'ADD_DATE';
        }
        if ($this->isColumnModified(AvailabilityTableMap::VALUE)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AvailabilityTableMap::ID)) $criteria->add(AvailabilityTableMap::ID, $this->id);
        if ($this->isColumnModified(AvailabilityTableMap::ADD_DATE)) $criteria->add(AvailabilityTableMap::ADD_DATE, $this->add_date);
        if ($this->isColumnModified(AvailabilityTableMap::VALUE)) $criteria->add(AvailabilityTableMap::VALUE, $this->value);
    ...
    {
        $criteria = new Criteria(AvailabilityTableMap::DATABASE_NAME);
        $criteria->add(AvailabilityTableMap::ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Component\Configuration\Model\Availability\Availability (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Component\Configuration\Model\Availability\Availability Clone of current object.
     * @throws PropelException
     */
    ...
    public function initAvailabilityI18ns($overrideExisting = true)
    {
        ...
        $this->collAvailabilityI18ns->setModel('\Gekosale\Component\Configuration\Model\Availability\AvailabilityI18n');
    }
