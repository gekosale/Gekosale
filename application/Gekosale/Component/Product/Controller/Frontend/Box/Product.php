 ...
 */
namespace Gekosale\Component\Product\Controller\Frontend;
...
class Product extends Box
{
    ...
    public function index ()
    {
        ...
        if (isset($this->product['idproduct'])){
            ...
            $deliveryPrice = ($this->product['discountprice'] != NULL) ? $this->product['discountprice'] : $this->product['price'];
            $delivery = App::getModel('delivery')->getDispatchmethodPriceForProduct($deliveryPrice, $this->product['weight']);
            ...
            $opinion = App::getContainer()->get('session')->getVolatileOpinionAdded();
            if ($opinion[0] == 1){
                $this->registry->template->assign('opinionadded', $this->trans('TXT_CLIENT_OPINION_ADDED'));
            }
            elseif ($opinion[0] == 2){
                $this->registry->template->assign('opinionadded', $this->trans('TXT_GUEST_OPINION_ADDED'));
            }
            if ($this->product['enable'] == 0 && (int) App::getContainer()->get('session')->getActiveUserid() > 0){
                $this->registry->template->assign('draft', $this->trans('TXT_PRODUCT_DRAFT'));
            }
