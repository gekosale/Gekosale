...
abstract class Producer implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Component\\Product\\Model\\Producer\\Map\\ProducerTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Component\Product\Model\Producer\Base\Producer object.
     * @see applyDefaults()
     */
     ...
     * @return   \Gekosale\Component\Product\Model\Producer\Producer The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ProducerTableMap::ID] = true;
        }
    ...
    public function setPhotoId($v)
    {
        ...
        if ($this->photo_id !== $v) {
            $this->photo_id = $v;
            $this->modifiedColumns[ProducerTableMap::PHOTO_ID] = true;
        }
    ...
    public function setIsEnabled($v)
    {
        ...
        if ($this->is_enabled !== $v) {
            $this->is_enabled = $v;
            $this->modifiedColumns[ProducerTableMap::IS_ENABLED] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ProducerTableMap::translateFieldName('IsEnabled', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = ProducerTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Gekosale\Component\Product\Model\Producer\Producer object", 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->producerI18nsScheduledForDeletion !== null) {
                if (!$this->producerI18nsScheduledForDeletion->isEmpty()) {
                    \Gekosale\Component\Product\Model\Producer\ProducerI18nQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ProducerTableMap::ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(ProducerTableMap::PHOTO_ID)) {
            $modifiedColumns[':p' . $index++]  = 'PHOTO_ID';
        }
        if ($this->isColumnModified(ProducerTableMap::IS_ENABLED)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProducerTableMap::ID)) $criteria->add(ProducerTableMap::ID, $this->id);
        if ($this->isColumnModified(ProducerTableMap::PHOTO_ID)) $criteria->add(ProducerTableMap::PHOTO_ID, $this->photo_id);
        if ($this->isColumnModified(ProducerTableMap::IS_ENABLED)) $criteria->add(ProducerTableMap::IS_ENABLED, $this->is_enabled);
    ...
    {
        $criteria = new Criteria(ProducerTableMap::DATABASE_NAME);
        $criteria->add(ProducerTableMap::ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Component\Product\Model\Producer\Producer (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Component\Product\Model\Producer\Producer Clone of current object.
     * @throws PropelException
     */
    ...
    public function initProducerI18ns($overrideExisting = true)
    {
        ...
        $this->collProducerI18ns->setModel('\Gekosale\Component\Product\Model\Producer\ProducerI18n');
    }
