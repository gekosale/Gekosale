...
abstract class Product implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Component\\Product\\Model\\Product\\Map\\ProductTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Component\Product\Model\Product\Base\Product object.
     * @see applyDefaults()
     */
     ...
     * @return   \Gekosale\Component\Product\Model\Product\Product The current object (for fluent API support)
     */
    ...
    public function setSortOrder($v)
    {
        ...
        if ($this->sort_order !== $v) {
            $this->sort_order = $v;
            $this->modifiedColumns[ProductTableMap::SORT_ORDER] = true;
        }
    ...
    public function setPhotoId($v)
    {
        ...
        if ($this->photo_id !== $v) {
            $this->photo_id = $v;
            $this->modifiedColumns[ProductTableMap::PHOTO_ID] = true;
        }
    ...
    public function setIsEnabled($v)
    {
        ...
        if ($this->is_enabled !== $v) {
            $this->is_enabled = $v;
            $this->modifiedColumns[ProductTableMap::IS_ENABLED] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ProductTableMap::translateFieldName('SortOrder', TableMap::TYPE_PHPNAME, $indexType)];
            $this->sort_order = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : ProductTableMap::translateFieldName('PhotoId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->photo_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : ProductTableMap::translateFieldName('IsEnabled', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException("Error populating \Gekosale\Component\Product\Model\Product\Product object", 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->productI18nsScheduledForDeletion !== null) {
                if (!$this->productI18nsScheduledForDeletion->isEmpty()) {
                    \Gekosale\Component\Product\Model\Product\ProductI18nQuery::create()
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(ProductTableMap::SORT_ORDER)) {
            $modifiedColumns[':p' . $index++]  = 'SORT_ORDER';
        }
        if ($this->isColumnModified(ProductTableMap::PHOTO_ID)) {
            $modifiedColumns[':p' . $index++]  = 'PHOTO_ID';
        }
        if ($this->isColumnModified(ProductTableMap::IS_ENABLED)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ProductTableMap::SORT_ORDER)) $criteria->add(ProductTableMap::SORT_ORDER, $this->sort_order);
        if ($this->isColumnModified(ProductTableMap::PHOTO_ID)) $criteria->add(ProductTableMap::PHOTO_ID, $this->photo_id);
        if ($this->isColumnModified(ProductTableMap::IS_ENABLED)) $criteria->add(ProductTableMap::IS_ENABLED, $this->is_enabled);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Component\Product\Model\Product\Product (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Component\Product\Model\Product\Product Clone of current object.
     * @throws PropelException
     */
    ...
    public function initProductI18ns($overrideExisting = true)
    {
        ...
        $this->collProductI18ns->setModel('\Gekosale\Component\Product\Model\Product\ProductI18n');
    }
    ...
    public function getCurrentTranslation(ConnectionInterface $con = null)
    {
         ...
         * @return   \Gekosale\Component\Product\Model\Product\ProductI18n The current object (for fluent API support)
         */
