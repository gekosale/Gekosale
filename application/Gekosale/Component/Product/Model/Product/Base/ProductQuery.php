...
namespace Gekosale\Component\Product\Model\Product\Base;

use \Exception;
use \PDO;
use Gekosale\Component\Product\Model\Product\Product as ChildProduct;
use Gekosale\Component\Product\Model\Product\ProductI18nQuery as ChildProductI18nQuery;
use Gekosale\Component\Product\Model\Product\ProductQuery as ChildProductQuery;
use Gekosale\Component\Product\Model\Product\Map\ProductTableMap;
 ...
 * @method     ChildProductQuery orderBySortOrder($order = Criteria::ASC) Order by the sort_order column
 * @method     ChildProductQuery orderByPhotoId($order = Criteria::ASC) Order by the photo_id column
 * @method     ChildProductQuery orderByIsEnabled($order = Criteria::ASC) Order by the is_enabled column
 *
 * @method     ChildProductQuery groupById() Group by the id column
 * @method     ChildProductQuery groupBySortOrder() Group by the sort_order column
 * @method     ChildProductQuery groupByPhotoId() Group by the photo_id column
 * @method     ChildProductQuery groupByIsEnabled() Group by the is_enabled column
 *
 ...
 * @method     ChildProduct findOneBySortOrder(int $sort_order) Return the first ChildProduct filtered by the sort_order column
 * @method     ChildProduct findOneByPhotoId(int $photo_id) Return the first ChildProduct filtered by the photo_id column
 * @method     ChildProduct findOneByIsEnabled(int $is_enabled) Return the first ChildProduct filtered by the is_enabled column
 *
 * @method     array findById(int $id) Return ChildProduct objects filtered by the id column
 * @method     array findBySortOrder(int $sort_order) Return ChildProduct objects filtered by the sort_order column
 * @method     array findByPhotoId(int $photo_id) Return ChildProduct objects filtered by the photo_id column
 * @method     array findByIsEnabled(int $is_enabled) Return ChildProduct objects filtered by the is_enabled column
 *
 */
abstract class ProductQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Gekosale\Component\Product\Model\Product\Base\ProductQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Gekosale\\Component\\Product\\Model\\Product\\Product', $modelAlias = null)
    {
    ...
    {
        if ($criteria instanceof \Gekosale\Component\Product\Model\Product\ProductQuery) {
            return $criteria;
        }
        $query = new \Gekosale\Component\Product\Model\Product\ProductQuery();
        if (null !== $modelAlias) {
    ...
    {
        $sql = 'SELECT ID, SORT_ORDER, PHOTO_ID, IS_ENABLED FROM product WHERE ID = :p0';
        try {
    ...
    public function filterBySortOrder($sortOrder = null, $comparison = null)
    {
        if (is_array($sortOrder)) {
            ...
            if (isset($sortOrder['min'])) {
                $this->addUsingAlias(ProductTableMap::SORT_ORDER, $sortOrder['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($sortOrder['max'])) {
                $this->addUsingAlias(ProductTableMap::SORT_ORDER, $sortOrder['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ProductTableMap::SORT_ORDER, $sortOrder, $comparison);
    }
    ...
    public function filterByPhotoId($photoId = null, $comparison = null)
    {
        if (is_array($photoId)) {
            ...
            if (isset($photoId['min'])) {
                $this->addUsingAlias(ProductTableMap::PHOTO_ID, $photoId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($photoId['max'])) {
                $this->addUsingAlias(ProductTableMap::PHOTO_ID, $photoId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ProductTableMap::PHOTO_ID, $photoId, $comparison);
    }
    ...
    public function filterByIsEnabled($isEnabled = null, $comparison = null)
    {
        if (is_array($isEnabled)) {
            ...
            if (isset($isEnabled['min'])) {
                $this->addUsingAlias(ProductTableMap::IS_ENABLED, $isEnabled['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($isEnabled['max'])) {
                $this->addUsingAlias(ProductTableMap::IS_ENABLED, $isEnabled['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ProductTableMap::IS_ENABLED, $isEnabled, $comparison);
    }

    /**
     * Filter the query by a related \Gekosale\Component\Product\Model\Product\ProductI18n object
     *
     * @param \Gekosale\Component\Product\Model\Product\ProductI18n|ObjectCollection $productI18n  the related object to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildProductQuery The current query, for fluid interface
     */
    ...
    {
        if ($productI18n instanceof \Gekosale\Component\Product\Model\Product\ProductI18n) {
            return $this
            ...
            throw new PropelException('filterByProductI18n() only accepts arguments of type \Gekosale\Component\Product\Model\Product\ProductI18n or Collection');
        }
    }
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Gekosale\Component\Product\Model\Product\ProductI18nQuery A secondary query class using the current class as primary query
     */
    public function useProductI18nQuery($relationAlias = null, $joinType = 'LEFT JOIN')
    {
        return $this
            ->joinProductI18n($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'ProductI18n', '\Gekosale\Component\Product\Model\Product\ProductI18nQuery');
    }
