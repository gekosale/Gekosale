...
use Gekosale\Plugin\Translation\Model\ORM\Translation;
use Gekosale\Plugin\Translation\Model\ORM\TranslationQuery;
...
class TranslationTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = 'Gekosale.Plugin.Translation.Model.ORM.Map.TranslationTableMap';
     ...
     */
    const OM_CLASS = '\\Gekosale\\Plugin\\Translation\\Model\\ORM\\Translation';

    /**
     * A class that can be returned by this tableMap
     */
    const CLASS_DEFAULT = 'Gekosale.Plugin.Translation.Model.ORM.Translation';
    ...
    public function initialize()
    {
        ...
        $this->setClassName('\\Gekosale\\Plugin\\Translation\\Model\\ORM\\Translation');
    ...
    {
        return $withPrefix ? TranslationTableMap::CLASS_DEFAULT : TranslationTableMap::OM_CLASS;
    }
    ...
    {
        $key = TranslationTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = TranslationTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + TranslationTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = TranslationTableMap::OM_CLASS;
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            TranslationTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = TranslationTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = TranslationTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(TranslationTableMap::DATABASE_NAME)->getTable(TranslationTableMap::TABLE_NAME);
    }
    ...
    {
      $dbMap = Propel::getServiceContainer()->getDatabaseMap(TranslationTableMap::DATABASE_NAME);
      if (!$dbMap->hasTable(TranslationTableMap::TABLE_NAME)) {
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            // rename for clarity
            $criteria = $values;
        } elseif ($values instanceof \Gekosale\Plugin\Translation\Model\ORM\Translation) { // it's a model object
            // create criteria based on pk values
            $criteria = $values->buildPkeyCriteria();
        } else { // it's a primary key, or an array of pks
            $criteria = new Criteria(TranslationTableMap::DATABASE_NAME);
            $criteria->add(TranslationTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = TranslationQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) { TranslationTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) { TranslationTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return TranslationQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Translation or Criteria object.
     *
     * @param mixed               $criteria Criteria or Translation object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Translation object
        }

        if ($criteria->containsKey(TranslationTableMap::COL_ID) && $criteria->keyContainsValue(TranslationTableMap::COL_ID) ) {
...
//
TranslationTableMap::buildTableMap();
