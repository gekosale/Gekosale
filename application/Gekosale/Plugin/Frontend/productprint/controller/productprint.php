...
class ProductPrintController extends Component\Controller
{
	...
	public function index ()
	{
		...
		if ( !empty($product['photo']['normal'][0]) ) {
			$product['photo']['normal'][0] = 'design/' . str_replace(DESIGNPATH, '', $product['photo']['normal'][0]);

			if ( !is_file(ROOTPATH . $product['photo']['normal'][0])) {
				unset($product['photo']['normal'][0]);
			}
		}
		...
		$htmlcontent = preg_replace_callback( '~(<img.+src=[\'"](.+?)[\'"].*?>)~', array( $this, 'parseImage'), $htmlcontent);
	...
	{
		if (strncmp('design/', $data[2], 7) === 0) {
			if ( is_file(ROOTPATH . $data[2])) {
				return $data[0];
			}
		}
		else if(strncmp(DESIGNPATH, $data[2], strlen(DESIGNPATH) - 7) === 0) {
			$file = ROOTPATH . substr($data[2], strlen(DESIGNPATH) - 7);
			if (is_file($file)) {
				return strtr($data[0], array(
					$data[2] => substr($data[2], strlen(DESIGNPATH) - 7)
				));
			}
		}
