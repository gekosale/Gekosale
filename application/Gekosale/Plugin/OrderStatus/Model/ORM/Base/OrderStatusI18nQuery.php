...
use \Exception;
use \PDO;
use Gekosale\Plugin\OrderStatus\Model\ORM\OrderStatusI18n as ChildOrderStatusI18n;
use Gekosale\Plugin\OrderStatus\Model\ORM\OrderStatusI18nQuery as ChildOrderStatusI18nQuery;
use Gekosale\Plugin\OrderStatus\Model\ORM\Map\OrderStatusI18nTableMap;
 ...
 * @method     ChildOrderStatusI18nQuery orderByName($order = Criteria::ASC) Order by the name column
 * @method     ChildOrderStatusI18nQuery orderByDefaultComment($order = Criteria::ASC) Order by the default_comment column
 *
 ...
 * @method     ChildOrderStatusI18nQuery groupByName() Group by the name column
 * @method     ChildOrderStatusI18nQuery groupByDefaultComment() Group by the default_comment column
 *
 ...
 * @method     ChildOrderStatusI18n findOneByName(string $name) Return the first ChildOrderStatusI18n filtered by the name column
 * @method     ChildOrderStatusI18n findOneByDefaultComment(string $default_comment) Return the first ChildOrderStatusI18n filtered by the default_comment column
 *
 ...
 * @method     array findByName(string $name) Return ChildOrderStatusI18n objects filtered by the name column
 * @method     array findByDefaultComment(string $default_comment) Return ChildOrderStatusI18n objects filtered by the default_comment column
 *
 */
abstract class OrderStatusI18nQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Gekosale\Plugin\OrderStatus\Model\ORM\Base\OrderStatusI18nQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Gekosale\\Plugin\\OrderStatus\\Model\\ORM\\OrderStatusI18n', $modelAlias = null)
    {
    ...
    {
        if ($criteria instanceof \Gekosale\Plugin\OrderStatus\Model\ORM\OrderStatusI18nQuery) {
            return $criteria;
        }
        $query = new \Gekosale\Plugin\OrderStatus\Model\ORM\OrderStatusI18nQuery();
        if (null !== $modelAlias) {
    ...
    {
        $sql = 'SELECT ID, LOCALE, NAME, DEFAULT_COMMENT FROM order_status_i18n WHERE ID = :p0 AND LOCALE = :p1';
        try {
    ...
    {
        $this->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(OrderStatusI18nTableMap::COL_LOCALE, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(OrderStatusI18nTableMap::COL_ID, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(OrderStatusI18nTableMap::COL_LOCALE, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    public function filterById($id = null, $comparison = null)
    {
        if (is_array($id)) {
            $useMinMax = false;
            if (isset($id['min'])) {
                $this->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $id['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($id['max'])) {
                $this->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $id['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $id, $comparison);
    }
    ...
    public function filterByLocale($locale = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(OrderStatusI18nTableMap::COL_LOCALE, $locale, $comparison);
    }
    ...
    public function filterByName($name = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(OrderStatusI18nTableMap::COL_NAME, $name, $comparison);
    }

    /**
     * Filter the query on the default_comment column
     *
     * Example usage:
     * <code>
     * $query->filterByDefaultComment('fooValue');   // WHERE default_comment = 'fooValue'
     * $query->filterByDefaultComment('%fooValue%'); // WHERE default_comment LIKE '%fooValue%'
     * </code>
     *
     * @param     string $defaultComment The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return ChildOrderStatusI18nQuery The current query, for fluid interface
     */
    public function filterByDefaultComment($defaultComment = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($defaultComment)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $defaultComment)) {
                $defaultComment = str_replace('*', '%', $defaultComment);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(OrderStatusI18nTableMap::COL_DEFAULT_COMMENT, $defaultComment, $comparison);
    }
    ...
    public function filterByOrderStatus($orderStatus, $comparison = null)
    {
        if ($orderStatus instanceof \Gekosale\Plugin\OrderStatus\Model\ORM\OrderStatus) {
            return $this
                ->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $orderStatus->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(OrderStatusI18nTableMap::COL_ID, $orderStatus->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
    ...
    {
        if ($orderStatusI18n) {
            $this->addCond('pruneCond0', $this->getAliasedColName(OrderStatusI18nTableMap::COL_ID), $orderStatusI18n->getId(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(OrderStatusI18nTableMap::COL_LOCALE), $orderStatusI18n->getLocale(), Criteria::NOT_EQUAL);
