...
abstract class BlogPhoto implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Plugin\\Blog\\Model\\ORM\\Map\\BlogPhotoTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Plugin\Blog\Model\ORM\Base\BlogPhoto object.
     * @see applyDefaults()
     */
    ...
    /**
     * Compares this with another <code>BlogPhoto</code> instance.  If
     * <code>obj</code> is an instance of <code>BlogPhoto</code>, delegates to
     * <code>equals(BlogPhoto)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return BlogPhoto The current object, for fluid interface
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[BlogPhotoTableMap::COL_ID] = true;
        }
    ...
    public function setBlogId($v)
    {
        ...
        if ($this->blog_id !== $v) {
            $this->blog_id = $v;
            $this->modifiedColumns[BlogPhotoTableMap::COL_BLOG_ID] = true;
        }
    ...
    public function setPhotoId($v)
    {
        ...
        if ($this->photo_id !== $v) {
            $this->photo_id = $v;
            $this->modifiedColumns[BlogPhotoTableMap::COL_PHOTO_ID] = true;
        }
     ...
     * @return   \Gekosale\Plugin\Blog\Model\ORM\BlogPhoto The current object (for fluent API support)
     */
    public function setIsMainPhoto($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : BlogPhotoTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : BlogPhotoTableMap::translateFieldName('BlogId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->blog_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : BlogPhotoTableMap::translateFieldName('PhotoId', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildBlogPhotoQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see BlogPhoto::setDeleted()
     * @see BlogPhoto::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        try {
            $deleteQuery = ChildBlogPhotoQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                BlogPhotoTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(BlogPhotoTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(BlogPhotoTableMap::COL_BLOG_ID)) {
    ...
    {
        $pos = BlogPhotoTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['BlogPhoto'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['BlogPhoto'][$this->getPrimaryKey()] = true;
        $keys = BlogPhotoTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    {
        $criteria = new Criteria(BlogPhotoTableMap::DATABASE_NAME);
        $criteria->add(BlogPhotoTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Plugin\Blog\Model\ORM\BlogPhoto (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Plugin\Blog\Model\ORM\BlogPhoto Clone of current object.
     * @throws PropelException
     */
