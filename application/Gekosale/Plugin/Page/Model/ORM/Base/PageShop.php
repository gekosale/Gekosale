...
abstract class PageShop implements ActiveRecordInterface 
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Plugin\\Page\\Model\\ORM\\Map\\PageShopTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Plugin\Page\Model\ORM\Base\PageShop object.
     */
    ...
    /**
     * Compares this with another <code>PageShop</code> instance.  If
     * <code>obj</code> is an instance of <code>PageShop</code>, delegates to
     * <code>equals(PageShop)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return PageShop The current object, for fluid interface
     */
     ...
     * @return   \Gekosale\Plugin\Page\Model\ORM\PageShop The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[PageShopTableMap::COL_ID] = true;
        }
    ...
    public function setPageId($v)
    {
        ...
        if ($this->page_id !== $v) {
            $this->page_id = $v;
            $this->modifiedColumns[PageShopTableMap::COL_PAGE_ID] = true;
        }
    ...
    public function setShopId($v)
    {
        ...
        if ($this->shop_id !== $v) {
            $this->shop_id = $v;
            $this->modifiedColumns[PageShopTableMap::COL_SHOP_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PageShopTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PageShopTableMap::translateFieldName('PageId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->page_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PageShopTableMap::translateFieldName('ShopId', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 3; // 3 = PageShopTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException("Error populating \Gekosale\Plugin\Page\Model\ORM\PageShop object", 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildPageShopQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see PageShop::setDeleted()
     * @see PageShop::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        try {
            $deleteQuery = ChildPageShopQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        try {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                PageShopTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
            $con->commit();
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PageShopTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(PageShopTableMap::COL_PAGE_ID)) {
            $modifiedColumns[':p' . $index++]  = 'PAGE_ID';
        }
        if ($this->isColumnModified(PageShopTableMap::COL_SHOP_ID)) {
            $modifiedColumns[':p' . $index++]  = 'SHOP_ID';
        }

        $sql = sprintf(
            'INSERT INTO page_shop (%s) VALUES (%s)',
    ...
    {
        $pos = PageShopTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['PageShop'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['PageShop'][$this->getPrimaryKey()] = true;
        $keys = PageShopTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PageShopTableMap::COL_ID)) $criteria->add(PageShopTableMap::COL_ID, $this->id);
        if ($this->isColumnModified(PageShopTableMap::COL_PAGE_ID)) $criteria->add(PageShopTableMap::COL_PAGE_ID, $this->page_id);
        if ($this->isColumnModified(PageShopTableMap::COL_SHOP_ID)) $criteria->add(PageShopTableMap::COL_SHOP_ID, $this->shop_id);
    ...
    {
        $criteria = new Criteria(PageShopTableMap::DATABASE_NAME);
        $criteria->add(PageShopTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Plugin\Page\Model\ORM\PageShop (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Plugin\Page\Model\ORM\PageShop Clone of current object.
     * @throws PropelException
     */
    ...
    {
        return (string) $this->exportTo(PageShopTableMap::DEFAULT_STRING_FORMAT);
    }
