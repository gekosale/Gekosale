<?php

/**
 * Gekosale, Open Source E-Commerce Solution
 * http://www.gekosale.pl
 *
 * Copyright (c) 2008-2012 Gekosale sp. z o.o.. Zabronione jest usuwanie informacji o licencji i autorach.
 *
 * This library is free software; you can redistribute it and/or 
 * modify it under the terms of the GNU Lesser General Public
 * License as published by the Free Software Foundation; either
 * version 2.1 of the License, or (at your option) any later version. 
 * 
 * 
 * $Revision: 619 $
 * $Author: gekosale $
 * $Date: 2011-12-19 22:09:00 +0100 (Pn, 19 gru 2011) $
 * $Id: news.php 619 2011-12-19 21:09:00Z gekosale $ 
 */
namespace Gekosale\Plugin;

use FormEngine;

class ProductCombinationForm extends Component\Form
{
	protected $populateData;

	public function setPopulateData ($Data)
	{
		$this->populateData = $Data;
	}

	public function initForm ()
	{
		$form = new FormEngine\Elements\Form(Array(
			'name' => 'productcombination',
			'action' => '',
			'method' => 'post'
		));
		
		$relatedProducts = $form->AddChild(new FormEngine\Elements\Fieldset(Array(
			'name' => 'related_products',
			'label' => $this->trans('TXT_SELECT_PRODUCTS')
		)));
		
		$products = $relatedProducts->AddChild(new FormEngine\Elements\ProductSelect(Array(
			'name' => 'products',
			'label' => $this->trans('TXT_SELECT_PRODUCTS'),
			'advanced_editor' => true,
			'repeat_min' => 1,
			'repeat_max' => FormEngine\FE::INFINITE
		)));
		
		$pricePane = $form->AddChild(new FormEngine\Elements\Fieldset(Array(
			'name' => 'price_pane',
			'label' => $this->trans('TXT_PRICE')
		)));
		
		$aggregator = $pricePane->AddChild(new FormEngine\Elements\ProductAggregator(Array(
			'name' => 'aggregator',
			'suffix' => $this->trans('TXT_CURRENCY'),
			'products_source_field' => $products
		)));
		
		$pricePane->AddChild(new FormEngine\Elements\TextField(Array(
			'name' => 'discount',
			'label' => $this->trans('TXT_DISCOUNT'),
			'suffix' => '%',
			'rules' => Array(
				new FormEngine\Rules\Required($this->trans('ERR_EMPTY_VALUE'))
			)
		)));
		
		$layerData = $form->AddChild(new FormEngine\Elements\Fieldset(Array(
			'name' => 'view_data',
			'label' => $this->trans('TXT_STORES')
		)));
		
		$layerData->AddChild(new FormEngine\Elements\LayerSelector(Array(
			'name' => 'view',
			'label' => $this->trans('TXT_VIEW'),
			'default' => Helper::getViewIdsDefault()
		)));
		
		$Data = Event::dispatch($this, 'admin.productcombination.initForm', Array(
			'form' => $form,
			'id' => (int) $this->registry->core->getParam(),
			'data' => $this->populateData
		));
		
		if (! empty($Data)){
			$form->Populate($Data);
		}
		
		$form->AddFilter(new FormEngine\Filters\Trim());
		$form->AddFilter(new FormEngine\Filters\Secure());
		
		return $form;
	}
}