...
abstract class Event implements ActiveRecordInterface 
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Gekosale\\Plugin\\Event\\Model\\ORM\\Map\\EventTableMap';
    ...
    /**
     * Initializes internal state of Gekosale\Plugin\Event\Model\ORM\Base\Event object.
     * @see applyDefaults()
     */
     ...
     *
     * @return Event The current object, for fluid interface
     */
     ...
     * @return   \Gekosale\Plugin\Event\Model\ORM\Event The current object (for fluent API support)
     */
    ...
    public function setModel($v)
    {
        ...
        if ($this->model !== $v) {
            $this->model = $v;
            $this->modifiedColumns[EventTableMap::COL_MODEL] = true;
        }
    ...
    public function setMethod($v)
    {
        ...
        if ($this->method !== $v) {
            $this->method = $v;
            $this->modifiedColumns[EventTableMap::COL_METHOD] = true;
        }
    ...
    public function setModule($v)
    {
        ...
        if ($this->module !== $v) {
            $this->module = $v;
            $this->modifiedColumns[EventTableMap::COL_MODULE] = true;
        }
    ...
    public function setHierarchy($v)
    {
        ...
        if ($this->hierarchy !== $v) {
            $this->hierarchy = $v;
            $this->modifiedColumns[EventTableMap::COL_HIERARCHY] = true;
        }
    ...
    public function setMode($v)
    {
        ...
        if ($this->mode !== $v) {
            $this->mode = $v;
            $this->modifiedColumns[EventTableMap::COL_MODE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EventTableMap::translateFieldName('Model', TableMap::TYPE_PHPNAME, $indexType)];
            $this->model = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EventTableMap::translateFieldName('Method', TableMap::TYPE_PHPNAME, $indexType)];
            $this->method = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EventTableMap::translateFieldName('Module', TableMap::TYPE_PHPNAME, $indexType)];
            $this->module = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 5 + $startcol : EventTableMap::translateFieldName('Hierarchy', TableMap::TYPE_PHPNAME, $indexType)];
            $this->hierarchy = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 6 + $startcol : EventTableMap::translateFieldName('Mode', TableMap::TYPE_PHPNAME, $indexType)];
            $this->mode = (null !== $col) ? (boolean) $col : null;
            $this->resetModified();
            ...
            throw new PropelException("Error populating \Gekosale\Plugin\Event\Model\ORM\Event object", 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(EventTableMap::COL_MODEL)) {
            $modifiedColumns[':p' . $index++]  = 'MODEL';
        }
        if ($this->isColumnModified(EventTableMap::COL_METHOD)) {
            $modifiedColumns[':p' . $index++]  = 'METHOD';
        }
        if ($this->isColumnModified(EventTableMap::COL_MODULE)) {
            $modifiedColumns[':p' . $index++]  = 'MODULE';
        }
        if ($this->isColumnModified(EventTableMap::COL_HIERARCHY)) {
            $modifiedColumns[':p' . $index++]  = 'HIERARCHY';
        }
        if ($this->isColumnModified(EventTableMap::COL_MODE)) {
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[6], $arr)) $this->setMode($arr[$keys[6]]);
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EventTableMap::COL_ID)) $criteria->add(EventTableMap::COL_ID, $this->id);
        if ($this->isColumnModified(EventTableMap::COL_NAME)) $criteria->add(EventTableMap::COL_NAME, $this->name);
        if ($this->isColumnModified(EventTableMap::COL_MODEL)) $criteria->add(EventTableMap::COL_MODEL, $this->model);
        if ($this->isColumnModified(EventTableMap::COL_METHOD)) $criteria->add(EventTableMap::COL_METHOD, $this->method);
        if ($this->isColumnModified(EventTableMap::COL_MODULE)) $criteria->add(EventTableMap::COL_MODULE, $this->module);
        if ($this->isColumnModified(EventTableMap::COL_HIERARCHY)) $criteria->add(EventTableMap::COL_HIERARCHY, $this->hierarchy);
        if ($this->isColumnModified(EventTableMap::COL_MODE)) $criteria->add(EventTableMap::COL_MODE, $this->mode);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Gekosale\Plugin\Event\Model\ORM\Event (or compatible) type.
     ...
     *
     * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return                 \Gekosale\Plugin\Event\Model\ORM\Event Clone of current object.
     * @throws PropelException
     */
