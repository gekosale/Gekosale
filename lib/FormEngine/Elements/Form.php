...
class Form extends Container
{
	...
	public function Render_JSAllegroParams ()
	{
		$render = "
			...
			<script type=\"text/javascript\" id=\"JSAllegroParams\">
				/*<![CDATA[*/
                                var JSAllegroParams = function() {
                                    $('#{$this->_attributes['name']}').GForm({
                                        sFormName: '{$this->_attributes['name']}',
                                        sClass: '{$this->_attributes['class']}',
                                        iTabs: " . (($this->_attributes['tabs'] == self::TABS_VERTICAL) ? 'GForm.TABS_VERTICAL' : 'GForm.TABS_HORIZONTAL') . ",
                                    ...
                                    $('#allegro-params-selector').mouseover();
                                };
                                /*var allegroParamsSelectStop = false;
                                $(document).delegate('#allegro-params-selector', 'mouseover', function(){
                                    if (!allegroParamsSelectStop && $('#allegro-params-selector select').length > 0) {
                                        $('#allegro-params-selector select').GSelect();
                                        allegroParamsSelectStop = true;
                                    }
                                });*/
				/*]]>*/
			</script>
