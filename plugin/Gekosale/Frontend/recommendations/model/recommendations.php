...
class RecommendationsModel extends Component\Model\Dataset
{

    public function getBlock ($limit, $view, $slot)
    {
        $this->registry->template->assign('dataset', $this->getPromotions($limit));
        $this->registry->template->assign('view', $view);

        return $this->registry->template->fetch('products.tpl');
    }

    public function getPromotions ($limit, $exclude = 0)
    {
    ...
    {
        return $this->getDataset('recommendations')->getDatasetRecords();
    }
}